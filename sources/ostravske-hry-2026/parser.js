...
function parseScore(linkText, matchTime, isLive) {
  // Před zápasem ostravskehry.cz zobrazuje jako odkaz čas startu (např. "11:15"),
  // během/po zápase zobrazuje skóre (např. "5:3"). Rozlišujeme podle shody s časem.
  // Probíhající zápas má v buňce navíc <span>Právě probíhá</span> — předané přes `isLive`.
...
export function parseMatches(root) {
  ...
  root.querySelectorAll('tr.match-row-desktop').forEach(tr => {
    ...
    // Pozn.: ostravskehry.cz občas u nevylosovaného play-off zápasu uvádí v obou
    // sloupcích týmů identický placeholder (viz SF-B id 2760: oba feeder sloty = stejný
    // 4F-B popisek). Není to chyba parseru — kopírujeme zdroj věrně. Duplikát řeší až
    // render (lib/bracket.js → nodeLabelStatic: "soupeř bude určen").
    ...
    const isLive = /Právě probíhá/i.test(tds[7].textContent || '');
    ...
    matches.push({
      ...
      home,
      away,
      score: parseScore(scoreText, time, isLive),
    });
  });

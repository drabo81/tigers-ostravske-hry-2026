...
export function resolvePlaceholder(cell, matches, table) {
  ...
  // Fallback A (spolehlivý): po odehrání zdrojové fáze jsou buňky reálná jména, token-set
  // lookup selže. Buňka vždy obsahuje právě jeden MH kód (H_n) — sleduj tu linii po
  // skutečných výsledcích až do zdrojové fáze. Funguje i když se los rozešel s idealizací
  // placeholderu (záleží jen na tom, kudy H-linie reálně prošla, ne na ostatních kódech).
  ...
  // Fallback B: seed-set heuristika — POUZE pro 2 tokeny (zdroj 16F-A = seed vs seed,
  // jednoznačné). Pro 4+ tokenů je nespolehlivá: složené kódy předpokládají idealizovaný
  // los a navíc 4F-B buňka může být buď poražený 8F-A, NEBO vítěz 8F-B (stejný formát) —
  // hádání by ukázalo špatný/nehraný tým. Takové buňky necháme neurčené.

...
export function resolvePlaceholder(cell, matches, table) {
  ...
  // Fallback: po odehrání zdrojové fáze ostravskehry.cz přepíše placeholdery reálnými
  // jmény, takže token-set lookup výše selže. Přelož všechny pozice na seed-jména týmů
  // a najdi zápas, jehož OBA účastníci do téhle množiny patří — to je jednoznačně
  // finálový zápas daného podstromu pavouka (vítězové postoupili právě z těchto pozic).
  if (!sourceMatch) {
...
function findMatchByCodeAndPhase(matches, phase, code, tigersCode = null, table = null) {
  ...
  // 3) Po dohrání skupin ostravskehry.cz nahradí placeholdery (H2, D3…) reálnými jmény,
  //    takže matchContainsCode už nic nenajde. Přelož kód na jméno týmu (z tabulky podle
  //    pořadí ve skupině) a najdi zápas dané fáze podle normalizovaného jména.
  if (table) {
    const teamName = resolveCode(code, table);
    if (teamName) {
      const norm = normalizeTeamName(teamName);
      return matches.matches.find(m => m.phase === phase &&
...
function nodeLabelStatic(template, match, table, tigersCode, matches) {
  ...
  if (homeLabel && awayLabel && homeLabel !== '?' && awayLabel !== '?') {
    if (homeLabel === awayLabel) {
      // Zdrojová data občas duplikují placeholder (oba feeder sloty B-větve ukazují na
      // stejný předchozí zápas), takže by tým hrál sám proti sobě. Soupeř je tím pádem
      // ještě neurčený — nezobrazuj duplikát.
      lines.push('soupeř bude určen');
    } else {

...
// B-větev (útěcha pro poražené 16F-A) v pořadí kol.
const B_CHAIN = ['8F-B', '4F-B', 'SF-B', 'FINAL-B'];

// Sleduje B-linii po skutečných výsledcích: vstupuje POŘAŽENÝ 16F-A na lince H_n, pak
// postupuje po vítězstvích (8F-B → 4F-B → SF-B → FINÁLE B). Vrací zápas cílové B-fáze,
// nebo undefined, pokud se tým ještě nedostal / linii nelze sledovat.
function findMatchOnLineB(matches, table, code, targetPhase) {
  const idx = B_CHAIN.indexOf(targetPhase);
  if (idx < 0) return undefined;
  const m16 = findMatchOnWinnerLineA(matches, table, code, '16F-A');
  if (!m16?.score) return undefined;
  let norm = normalizeTeamName(m16.score.home > m16.score.away ? m16.away : m16.home); // poražený 16F
  let match;
  for (let i = 0; i <= idx; i++) {
    const ph = B_CHAIN[i];
    match = matches.matches.find(m => m.phase === ph &&
...
function findMatchByCodeAndPhase(matches, phase, code, tigersCode = null, table = null) {
  ...
  // 2) Hledej podle position kódu v placeholderu (náhled budoucích kol, dokud fáze drží
  //    placeholdery). Přeskoč pro 4F-B/SF-B/FINÁLE B: jejich kódy odkazují na A-linii
  //    (vítěze 16F-A), což je pro B-větev nereálné a navíc bývají duplicitní — věřit jim
  //    by dalo špatný zápas. Pro tyto fáze rozhoduje až B-walk podle skutečných výsledků.
  if (!['4F-B', 'SF-B', 'FINAL-B'].includes(phase)) {
  ...
  if (table) {
    if (A_CHAIN.includes(phase)) {
      ...
      if (m) return m;
    } else if (B_CHAIN.includes(phase)) {
      // B-větev: vstupuje poražený 16F-A na lince H_n, pak postupuje po vítězstvích.
      const m = findMatchOnLineB(matches, table, code, phase);
      if (m) return m;
    } else {
